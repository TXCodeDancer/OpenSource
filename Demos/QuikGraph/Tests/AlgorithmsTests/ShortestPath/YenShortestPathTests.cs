...
{
    public class YenShortestPathTests
    {
        [Theory]
        ...
        public void Test0(string inputFile)
        {
            ...
            var outputFile = @$"{directory}\ShortestPath\Yen\{file}";
            ...
            var root = nodes[0];
            var target = nodes[nodes.Count - 1];
            List<string> actual = ShortestPathHelper.YenShortestPathHelper(nodes, edges, root, target);
        ...
        public void Test1(string inputFile)
        {
            string directory = Path.GetDirectoryName(inputFile);
            string file = Path.GetFileNameWithoutExtension(inputFile);
            var outputFile = @$"{directory}\ShortestPath\Yen\AllPaths\{file}";
            var expectedfile = @$"{outputFile}.a";
            var resultsfile = $"{outputFile}.r";
            ...
            var root = nodes[0];
            var target = nodes[nodes.Count - 1];
            List<string> actual = ShortestPathHelper.YenAllPathsHelper(nodes, edges, root, target);
            File.WriteAllLines(resultsfile, actual);
