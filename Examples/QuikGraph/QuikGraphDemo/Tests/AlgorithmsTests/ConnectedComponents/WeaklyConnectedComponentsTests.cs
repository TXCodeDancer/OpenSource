...
namespace Tests
{
    public class WeaklyConnectedComponentsTests
    {
        ...
        public void Test0(string inputFile)
        {
            ...
            var outputFile = @$"{directory}\ConnectedComponents\WeaklyConnectedComponents\{file}";
            ...
            List<string> actual = ConnectedComponentsHelper.WeaklyConnectedComponents(nodes, edges);
        ...
        public void Test1(string inputFile)
        {
            string directory = Path.GetDirectoryName(inputFile);
            string file = Path.GetFileNameWithoutExtension(inputFile);
            var outputFile = @$"{directory}\ConnectedComponents\WeaklyConnectedComponentsGraphs\{file}";
            var expectedfile = @$"{outputFile}.a";
            var resultsfile = $"{outputFile}.r";
