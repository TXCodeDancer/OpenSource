...
namespace Tests
{
    public class DFSEdgePredecessorRecorderTests
    {
        [Theory]
        [MemberData(nameof(GetInputFiles))]
        public void Test0(string inputFile)
        {
            var expectedfile = $"{inputFile}.epr_a";
            var resultsfile = $"{inputFile}.epr_r";
            ...
            List<string> actual = Program.DFSEdgePredecessorRecorderHelper(nodes, edges, inputFile);
